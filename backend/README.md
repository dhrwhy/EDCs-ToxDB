...
Endocrine Disrupting Chemicals Toxicogenomics Database（EDC-ToxDB）的后端服务，基于 FastAPI 构建，提供 RESTful API 供前端调用。
...
│   ├── models/
│   │   ├── __init__.py
│   │   ├── main_record.py     # MainRecord ORM 模型（映射 main_records 表，51 列：4 系统字段 + 47 Excel 源字段）
...
│   ├── routers/
│   │   ├── __init__.py
│   │   ├── stats.py           # GET /api/stats/summary, /api/stats/publication-years, /api/stats/organ-distribution, /api/stats/mesh-tree
...
| `EXCEL_PATH` | `/data/source_excel/260320小鼠双端信息新版本.xlsx` | 原始 Excel 文件路径 |
...
- 两张表：`main_records`（986 行，51 列：4 系统字段 + 47 Excel 源字段）、`record_assets`（642 行，13 字段）
...
| 2 | `GET /api/stats/publication-years` | 按发表年份统计文献数量 | 首页统计总览 / 统计页 |
| 3 | `GET /api/stats/organ-distribution` | 器官/组织分布统计 | 首页统计总览 / 统计页 |
| 4 | `GET /api/stats/mesh-tree` | MESH 分类树状结构 | 统计页 |
| 5 | `GET /api/search` | 全局搜索（分类 + 分页） | 首页搜索 / 搜索结果页 |
| 6 | `GET /api/search/export` | 导出搜索结果为 Excel | 搜索结果页 |
| 7 | `GET /api/browse` | 浏览分析条目（筛选 + 排序 + 分页） | 浏览页 |
| 8 | `GET /api/browse/filters` | 获取浏览页筛选选项 | 浏览页 |
| 9 | `GET /api/analysis/{analysis_key}` | 分析条目详情（summary + samples + assets） | 详情页 |
| 10 | `GET /api/analysis/{analysis_key}/tables/deg-table` | DEG 差异基因表（TSV 解析 + 分页） | 详情页差异分析面板 |
| 11 | `GET /api/analysis/{analysis_key}/assets` | 分析条目资源列表 | 详情页 |
| 12 | `GET /api/assets/{asset_id}/download` | 下载单个资源文件（PDF/TXT） | 详情页 |
| 13 | `GET /api/download/database` | 下载整库 Excel | 下载页 |
...
返回 Excel 文件流。文件名：`EDC-ToxDB_搜索结果_YYYYMMDD_HHMMSS.xlsx`
...
| organism | string | 否 | 物种筛选 |
| in_vivo_vitro | string | 否 | 体内/体外筛选 |
| strain | string | 否 | 品系/细胞系筛选 |
| gender | string | 否 | 性别筛选 |
| year_min | int | 否 | 发表年份最小值 |
| year_max | int | 否 | 发表年份最大值 |
| sort_by | string | 否 | 排序字段（如 deseq_id, chemical_name 等） |
...
```json
{
  ...
  "in_vivo_vitro_options": ["in vivo", "in vitro"],
  "strains": ["C57BL/6", "CD-1", "..."],
...
| 6 | GET /api/browse/filters | 7 组织分类, 2010-2025 |
